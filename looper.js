...
function windowDisplay() {
  ...
  submitButton.onClick = function() {
    FolderLooper(source, extraPrints, space.text);
    return w.close();
  }
...
var quantity = RemainingPrintQTY;
...
var docHeight = (rows) * points(height + space);
  ...
  var qtyPerSheet = RemainingPrintQTY
}
...
var doc = app.documents.add(
  ...
  if (sheetsNeeded > 0) {
    newFile(qtyPerSheet, width, height, space, canvasWidth, canvasHeight, filePath, infoPath, destination, batch, columns, sheetCount, qtyPerSheet, rows)
  }
...
function FolderLooper(srcFolder, extraPrints, space) {
  ...
  for (var i = 0; i < allPrintPDFs.length; i++) {
    ...
    var rows = Math.floor(points(maxDocHeight) / points(itemSpecs.Height + space))
    var columns = Math.floor( canvasWidth / ( points(itemSpecs.Width) + points(space) ) )
